...
function isRpcRequestBody(
  x: unknown,
): x is { fnName: string; args: unknown[] } {
...
//
// Deliberately not annotated `: Plugin` (from vite's own types) — a
// consumer's project installs its own copy of `vite`, separate from the one
// in this package's node_modules, and vite's `Plugin` type carries private
// fields (`_pluginContextMap`, etc. on nested types like DevEnvironment)
// that make two structurally-identical `vite` installs nominally
// incompatible. Annotating the return type here forces TS to check the
// returned object against *our* `vite` install's `Plugin`, which then fails
// to satisfy the *consumer's* `vite`'s `PluginOption` in their vite.config.ts
// (verified: reproduces a "Types have separate declarations of a private
// property '_pluginContextMap'" error). Leaving the return type inferred
// avoids the cross-package nominal check entirely; the object shape itself
// is still structurally a valid Vite plugin.
  ...
  return {
    ...
    async configureServer(server: ViteDevServerLike) {
      ...
      const fileConfig = options.srcDir
        ? undefined
        : await loadGasPConfig(
            options.configFile ?? join(server.config.root, "gas-p.config.ts"),
          );
      ...
      const services: ServiceOptions = {
        ...
        clientSecretPath:
          options.clientSecretPath ?? DEFAULT_CLIENT_SECRET_PATH,
      ...
      const consumerConfig: ConsumerViteConfig = {
        ...
        plugins: (server.config.plugins ?? []).filter(
          isConsumerPlugin,
        ) as ConsumerViteConfig["plugins"],
      };
      ...
      const fixturesFile =
        options.fixturesFile ?? join(server.config.root, "gas-p.fixtures.ts");
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        const { html, xFrameOptionsMode } = await source.renderDoGet(
      ...
      server.middlewares.use(endpoint, async (req, res, next) => {
        ...
        if (!isRpcRequestBody(parsed)) {
          ...
          res.end(
            JSON.stringify({
              ok: false,
              error: { message: "Malformed RPC request body" },
            }),
          );
          return;
        }
