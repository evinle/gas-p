...
describe('renderDoGet', () => {
  it('renders the HTML file returned by doGet unchanged when it has no scriptlets', async () => {
    const html = await renderDoGet({ srcDir: join(FIXTURES, 'plain-html') });
    expect(html).toBe(
      ['<html>', '  <body>', '    <h1>Hello gas-p</h1>', '  </body>', '</html>', ''].join('\n')
    );
  });

  it('HTML-escapes <?= ?> scriptlet output', async () => {
    const html = await renderDoGet({ srcDir: join(FIXTURES, 'escaped-scriptlet') });
    expect(html).toBe('<p>Hello, &lt;b&gt;World&lt;/b&gt;!</p>\n');
  });

  it('does not escape <?!= ?> scriptlet output', async () => {
    const html = await renderDoGet({ srcDir: join(FIXTURES, 'unescaped-scriptlet') });
    expect(html).toBe('<p>Hello, <b>World</b>!</p>\n');
  });

  it('strips <?# ?> scriptlet comments entirely from the output', async () => {
    const html = await renderDoGet({ srcDir: join(FIXTURES, 'comment-scriptlet') });
    expect(html).toBe('<p>Hello, World!</p>\n');
  });

  it('renders a plain HtmlOutput from createHtmlOutputFromFile().setTitle(), matching the real v-reimburse doGet() pattern', async () => {
    const html = await renderDoGet({ srcDir: join(FIXTURES, 'html-output') });
    expect(html).toBe(
      ['<html>', '  <body>', '    <h1>VReimbursement Portal</h1>', '  </body>', '</html>', ''].join('\n')
    );
  });

  it('injects addMetaTag/setFaviconUrl tags into an existing <head>, matching real Apps Script serving behavior', async () => {
    const html = await renderDoGet({ srcDir: join(FIXTURES, 'meta-favicon-with-head') });
    expect(html).toBe(
      [
  ...
  it('creates and prepends a <head> when addMetaTag is used but the content has no <head> of its own', async () => {
    const html = await renderDoGet({ srcDir: join(FIXTURES, 'meta-favicon-no-head') });
    expect(html).toBe('<head><meta name="viewport" content="width=device-width"/></head><p>hi</p>');
  });

  it('does not persist module-level state across separate renderDoGet calls', async () => {
    const counterDir = join(FIXTURES, 'counter');
    const first = await renderDoGet({ srcDir: counterDir });
    const second = await renderDoGet({ srcDir: counterDir });
    expect(first).toBe('<p>Count: 1</p>\n');
    expect(second).toBe('<p>Count: 1</p>\n');
  });

  it('handles escaped, unescaped, and comment scriptlets together in one template', async () => {
    const html = await renderDoGet({ srcDir: join(FIXTURES, 'combined-scriptlets') });
    expect(html).toBe(
      [
  ...
  it('throws a ReferenceError when doGet references an undeclared global, matching real Apps Script', async () => {
    await expect(renderDoGet({ srcDir: join(FIXTURES, 'undeclared-global') })).rejects.toThrow(ReferenceError);
    await expect(renderDoGet({ srcDir: join(FIXTURES, 'undeclared-global') })).rejects.toThrow(/someUndeclaredService is not defined/);
  });
});
