...
| Service | Notes |
|---|---|
| `CalendarApp` | Backed by Google Calendar API v3. `getCalendarById`/`getDefaultCalendar` gated by `devResourceIds`. Live mode covers only `getEvents()`/`createEvent()` — see the note below on the deliberate gap with `Calendar` |
| `Calendar`, `People`, `Docs`, `Tasks`, `Sheets`, `BigQuery`, `Drive` | Advanced services (`enabledAdvancedServices` in `appsscript.json`). Stub-only — every method throws `GasPNotImplementedError` until a matching [Declared Fixture](#gas-pfixturests--optional-declared-fixtures-for-local-mode) answers it; no Live mode yet. `Drive` is distinct from the basic `DriveApp` for the same reason `Calendar` is distinct from `CalendarApp` — see the note below |
...
**`CalendarApp` vs. `Calendar` — a deliberate gap, not an oversight.** Both talk to the same underlying Google Calendar API v3 `events` resource, but they cover it differently on purpose:

- `CalendarApp` is the basic service. Its Live mode is a small, hand-written surface — `getEvents()`/`createEvent()` — built directly on that one resource's `list`/`insert` calls.
- `Calendar` is the advanced service. It exposes the resource's full REST method surface (`Events.patch`, `.move`, `.watch`, ...) plus every other Calendar API collection (`Acl`, `CalendarList`, `Freebusy`, `Settings`, ...), but is currently stub-only in every mode — no Live-mode bridging yet, only Declared Fixtures.

So a capability like setting an attendee's `optional: true` flag (which needs `Events.patch`, not `Events.insert`) is only reachable through `Calendar`, and only in Local mode via a fixture — `CalendarApp` can't express it, and `Calendar` can't yet reach Google for it. The same split applies to `Drive` alongside the basic `DriveApp` (e.g. custom permission roles or `Changes.list`, neither expressible through `DriveApp`), and to `People`/`Docs`/`Tasks`/`Sheets`/`BigQuery`, none of which have a basic-service counterpart at all yet. Building real Live-mode bridging for the advanced services is its own, larger piece of work (the object-per-call subprocess bridge `CalendarApp` uses doesn't translate directly to these REST-call-shaped surfaces) — tracked separately from the stub registration itself.
