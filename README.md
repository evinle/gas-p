...
| `Calendar`, `People` | The advanced Calendar and People services (`enabledAdvancedServices` in `appsscript.json`). Stub-only — every method throws `GasPNotImplementedError` until a matching [Declared Fixture](#gas-pfixturests--optional-declared-fixtures-for-local-mode) answers it; no Live mode yet, see the note below |
...
So a capability like setting an attendee's `optional: true` flag (which needs `Events.patch`, not `Events.insert`) is only reachable through `Calendar`, and only in Local mode via a fixture — `CalendarApp` can't express it, and `Calendar` can't yet reach Google for it. The same split applies to `People` alongside any future basic People-shaped service. Building real Live-mode bridging for the advanced services is its own, larger piece of work (the object-per-call subprocess bridge `CalendarApp` uses doesn't translate directly to `Calendar`/`People`'s REST-call-shaped surface) — tracked separately from the stub registration itself.
